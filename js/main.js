...
jQuery(document).ready(function($) {
	...
	var siteCountDown = function() {
		...
		if ( $('#date-countdown').length > 0 ) {
			$('#date-countdown').countdown('2022/08/15', function(event) {
			  var $this = $(this).html(event.strftime(''
			    + '<span class="countdown-block"><span class="label">%w</span> недель </span>'
			    + '<span class="countdown-block"><span class="label">%d</span> день </span>'
...
$('.btn').click(function(){
  $(".block_with_text").slideToggle(100);
});  


window.addEventListener('load', function() {
    var image = document.querySelector(".image");
    image.classList.add("image_vis");
});
